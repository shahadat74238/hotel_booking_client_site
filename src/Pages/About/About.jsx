import { Helmet } from "react-helmet-async";

const About = () => {
  return (
    <div className="min-h-screen">
      <Helmet>
        <title className="uppercase">Five Star | About</title>
      </Helmet>
      <div className="h-[50vh] lg:h-screen w-full bg-[url('https://fivestar.qodeinteractive.com/wp-content/uploads/2017/05/About-us-title-img.jpg')] bg-no-repeat bg-center bg-cover">
        <div className="h-full bg-black/40 w-full flex items-center justify-center">
          <div>
            <h1 className="lg:text-2xl text-lg text-center text-f-color mb-3">
              Welcome to FiveStar
            </h1>
            <h1
              style={{ letterSpacing: "5px" }}
              className="text-2xl text-center md:text-5xl font-bold uppercase text-white"
            >
              About US
            </h1>
            <hr className="lg:my-8 md:my-4 my-2 w-60 mx-auto border border-f-color" />
            
          </div>
        </div>
      </div>
    </div>
  );
};

export default About;
